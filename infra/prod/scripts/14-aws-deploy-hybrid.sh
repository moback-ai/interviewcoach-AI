#!/usr/bin/env bash
# Deploy PROD Hybrid stack (ALB + ASG c6i.xlarge + ElastiCache) and cut over.
#
# Usage: CONFIRM=YES bash infra/prod/scripts/14-aws-deploy-hybrid.sh
set -euo pipefail
...
HYBRID_STACK="${HYBRID_STACK_NAME:-interviewcoach-prod-hybrid}"
PROXY_STACK="${RDS_PROXY_STACK_NAME:-interviewcoach-prod-rds-proxy}"
HYBRID_TEMPLATE="$SCRIPT_DIR/../cloudformation/prod-hybrid-stack.yaml"
...
IMAGE_TAG="${IMAGE_TAG:-prod-20260630b}"
INSTANCE_TYPE="${HYBRID_INSTANCE_TYPE:-c6i.xlarge}"
CF_DIST_ID="${CF_DIST_ID:?}"
PROD_ENV="$SCRIPT_DIR/../prod.env"

if [[ "${CONFIRM:-}" != "YES" ]]; then
  echo "This deploys PROD Hybrid: ALB + ASG (${INSTANCE_TYPE}) + ElastiCache + RDS Multi-AZ + RDS Proxy."
  echo "CloudFront API origin will switch from single EC2 to ALB."
  echo "Re-run: CONFIRM=YES bash $0"
  exit 1
fi

echo "=== Step 1: Hybrid stack (ALB + ASG + ElastiCache) ==="
aws cloudformation deploy \
  --region "$REGION" \
  --stack-name "$HYBRID_STACK" \
  --template-file "$HYBRID_TEMPLATE" \
  --parameter-overrides \
    VpcId="$VPC_ID" \
    ...
    ImageTag="$IMAGE_TAG" \
    DesiredCapacity="${HYBRID_DESIRED_CAPACITY:-2}" \
    MinSize="${HYBRID_MIN_SIZE:-2}" \
    MaxSize="${HYBRID_MAX_SIZE:-4}" \
    RdsSecurityGroupId="$RDS_SG" \
  --no-fail-on-empty-changeset

ALB_DNS=$(aws cloudformation describe-stacks --region "$REGION" --stack-name "$HYBRID_STACK" \
  --query "Stacks[0].Outputs[?OutputKey=='AlbDnsName'].OutputValue" --output text)
REDIS_URL=$(aws cloudformation describe-stacks --region "$REGION" --stack-name "$HYBRID_STACK" \
  --query "Stacks[0].Outputs[?OutputKey=='RedisUrl'].OutputValue" --output text)
API_SG=$(aws cloudformation describe-stacks --region "$REGION" --stack-name "$HYBRID_STACK" \
  --query "Stacks[0].Outputs[?OutputKey=='ApiSecurityGroupId'].OutputValue" --output text)
ASG_NAME=$(aws cloudformation describe-stacks --region "$REGION" --stack-name "$HYBRID_STACK" \
...
export INFRA_LAYOUT="hybrid-asg"
...
for i in $(seq 1 40); do
  HEALTHY=$(aws elbv2 describe-target-health --region "$REGION" \
    --target-group-arn "$(aws cloudformation describe-stacks --region "$REGION" --stack-name "$HYBRID_STACK" \
...
python3 - "$PROD_ENV" "$ALB_DNS" "$REDIS_URL" "$PROXY_HOST" "$ASG_NAME" "$API_SG" "$HYBRID_STACK" <<'PY'
import sys
path, alb, redis_url, proxy, asg, api_sg, stack = sys.argv[1:8]
updates = {
    "INFRA_LAYOUT": "hybrid-asg",
    "HYBRID_STACK_NAME": stack,
    "ALB_DNS_NAME": alb,
    "API_ORIGIN_DOMAIN": alb,
    "REDIS_URL": redis_url,
    "DB_HOST": proxy,
    "HYBRID_ASG_NAME": asg,
    "HYBRID_API_SG_ID": api_sg,
    "HYBRID_INSTANCE_TYPE": "c6i.xlarge",
...
echo ""
echo "=== Hybrid deploy complete ==="
echo "  ALB:         http://${ALB_DNS}/api/health"
echo "  Site:        https://www.ugaanlabs.ai/api/health (after CF propagation)"
echo "  Redis:       $REDIS_URL"
echo "  DB (proxy):  $PROXY_HOST"
echo ""
echo "Old single EC2 (${API_INSTANCE_ID:-}) is still running for rollback."
echo "After 24h stable: terminate old instance and update alarms to ASG."
